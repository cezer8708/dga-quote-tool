...
def render_welcome_splash() -> None:
    st.markdown(
        ...
        <style>
            ...
            .welcome-hero {
                ...
                margin-bottom: 18px;
                background:
                    radial-gradient(circle at top right, rgba(54, 124, 255, 0.16), transparent 34%),
    ...
    logo_col, brand_col = st.columns([0.85, 2.35], gap="medium")
    with logo_col:
        if APP_LOGO_PATH:
            st.image(APP_LOGO_PATH, width=185)
    with brand_col:
        st.markdown(
            """
