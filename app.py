...
def main_app():
    ...
    import pandas as pd
    import streamlit as st
    # Requires reportlab for PDF generation (pip install reportlab)
    try:
        ...
        st.error("🚨 PDF generation library 'reportlab' not found. Please run: pip install reportlab")

        # Define placeholder classes/funcs to prevent app crash
    ...
    def load_products(path: str = "products.csv") -> pd.DataFrame:
        ...
        try:
            ...
            # --- CRITICAL FIX 2: Ensure all SKUs are stripped and UPPERCASE ---
            # This resolves case-sensitivity and hidden whitespace errors in the lookup key
            df["SKU"] = df["SKU"].astype(str).str.strip().str.upper()
            df["Name"] = df["Name"].astype(str).str.strip()
            # --- END CRITICAL FIX 2 ---
    ...
    def is_basket_5_7_X(item: dict) -> bool:
        # Ensure item SKU is checked against the normalized, uppercase SKU from the data frame
        sku = (item.get("sku") or "").upper().strip()
        name = (item.get("name") or "").lower()
        if sku in ALLOW_COURSE_SKUS:
            return True
    ...
    def build_pdf(buffer: io.BytesIO, customer: dict, items: list, fees: dict, totals: dict,
        ...
        # Check for reportlab classes
        if 'SimpleDocTemplate' not in globals() or SimpleDocTemplate == SimpleDocTemplate.__class__:
            print("Reportlab not available. Cannot generate PDF.", file=sys.stderr)
            return b""

        try:
            doc = SimpleDocTemplate(buffer, pagesize=letter, rightMargin=36, leftMargin=36, topMargin=30,
                                    bottomMargin=30)
            styles = getSampleStyleSheet()
            ...
            styles.add(ParagraphStyle('LeftInfo', parent=styles['Normal'], fontSize=10, leading=12, alignment=TA_LEFT))
            styles.add(
                ParagraphStyle('QuoteHeaderTitle', parent=styles['Heading2'], alignment=TA_RIGHT, fontSize=14,
                               leading=16))

            story = []

            notes_style = ParagraphStyle("LineNote", parent=styles["Normal"], fontSize=8, leading=10,
                                         textColor=colors.grey,
                                         leftIndent=6)
            notes_style_2 = ParagraphStyle("LineNote2", parent=styles["Normal"], fontSize=8, leading=10,
            ...
            if template == "order":
                ...
                t_sub = Table([
                    ["Subtotal:", fmt_money(totals.get('subtotal', 0.0))],
                    ["Drop-Ship Fee:", fmt_money(fees.get('drop_ship_fee', 0.0))],
                    [f"Sales Tax ({totals.get('tax_rate_pct', 0.0) * 100:.2f}%):",
                     fmt_money(totals.get('sales_tax', 0.0))],
                ], colWidths=[sub_tbl_w * 0.6, sub_tbl_w * 0.4])
                t_sub.setStyle(TableStyle([
                    ('GRID', (0, 0), (-1, -1), 0.5, colors.black),
                    ('ALIGN', (1, 0), (-1, -1), 'RIGHT'),
                ...
                acc_width = 3.5 * inch
                # The remaining PDF table for accessories was incomplete in the provided chunks.
                # I'll complete it and add the totals.
                acc_data = [
                    [Paragraph("<b>Additional Course Equipment to Consider*</b>",
                               ParagraphStyle('ACCHdr', parent=styles['Normal'], fontSize=9, alignment=TA_CENTER,
                                              textColor=colors.black, leading=11), colSpan=2)],
                    ["Number Plate", fmt_money(35.00)],
                    ["Powder Coat Fee - Stock Color", fmt_money(90.00)],
                    ["Additional Anchor - Pin Positions", fmt_money(30.00)],
                    ["Basic Color Tee Sign", fmt_money(55.00)],
                    ...
                    ["Basket Flag - New Product", fmt_money(30.00)],
                    [Paragraph("<b>*Per Unit Pricing</b>",
                               ParagraphStyle('ACCfTR', parent=styles['Normal'], fontSize=8, alignment=TA_CENTER,
                                              textColor=colors.black, leading=10), colSpan=2)],
                ]
                acc_tbl = Table(acc_data, colWidths=[acc_width * 0.7, acc_width * 0.3])
                acc_tbl.setStyle(TableStyle([
                    ('GRID', (0, 0), (-1, -1), 0.5, colors.black),
                    ...
                    ('SPAN', (0, -1), (1, -1)),  # Span footer
                    ('ALIGN', (1, 1), (1, -2), 'RIGHT'),
                    ('ALIGN', (0, 0), (0, 0), 'CENTER'),
                    ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'),
                ]))

                # Totals Table for Quote
                ...
                final_section = Table([[acc_tbl, t_totals]],
                                      colWidths=[acc_width + 0.3 * inch, totals_width + 0.3 * inch])
                final_section.setStyle(TableStyle([
                    ('LEFTPADDING', (0, 0), (-1, -1), 0), ('RIGHTPADDING', (0, 0), (-1, -1), 0),
                    ('TOPPADDING', (0, 0), (-1, -1), 0), ('BOTTOMPADDING', (0, 0), (-1, -1), 0),
                    ('VALIGN', (0, 0), (-1, -1), 'TOP'),
                    ('ALIGN', (1, 0), (1, 0), 'RIGHT')
                ]))
                story += [final_section, Spacer(1, 12)]

            # Footer Notes
            story += [Paragraph(footer_notes_text.replace('\n', '<br/>'), notes_style_2)]
            story += [Spacer(1, 12)]
    ...
    def main_app():
        ...
        # We use .upper() here to ensure the SKU selected from the dropdown
        # matches the normalized SKU in the DataFrame.
        sku_to_name = products_df.set_index("SKU")["Name"].to_dict()
        ...
        with st.expander("Product Data Debug Check (UNIT PRICES)", expanded=False):
            ...
            if not non_zero_products.empty:
                ...
                st.success(
                    f"✅ Prices are loading correctly! Showing {len(non_zero_products)} products with price > $0.00.")
            else:
        ...
        for i, row in enumerate(st.session_state["line_items"]):
            ...
            # --- CRITICAL FIX 3: Normalize SKU for Index Lookup ---
            current_sku = row.get("sku") or ""
            normalized_current_sku = current_sku.strip().upper() if current_sku != "(custom)" else "(custom)"

            try:
                sel_idx = sku_options.index(normalized_current_sku)
            except ValueError:
                sel_idx = 0
            # --- END CRITICAL FIX 3 ---
            ...
            with c1:
                ...
                if sku_selected == "(custom)":
                    ...
                    # sku_selected is already normalized (UPPERCASE, stripped)
        ...
        if pdf_col2.button("Process as Order / PO", use_container_width=True, type="secondary"):
            ...
            pdf_data_order = build_pdf(pdf_buffer_order, st.session_state["customer"], st.session_state["line_items"],
                                       fees,
                                       totals,
